...
printf {src in City: dc[src] = 1}:
    "Town with MINIMUM cost (DC): %s (%dEUR)\n", src,
    sum {dst in City} cost[src,dst] * deliver[src,dst];
printf "*********************************************************************\n";
printf {src in City, dst in City:
    dc[src] = 1 and
    cost[src,dst] = max {_dst in City} cost[src,_dst] * deliver[src,_dst]}:
        "Town with LARGEST DELIVERY cost: %s (%dEUR) [from %s]\n", dst, cost[src,dst], src;
printf "*********************************************************************\n\n";
